...
run_test() {
    ...
    # Determine scaling based on test type or custom value
    local NUM_WORKERS
    if [ ! -z "$CUSTOM_WORKERS" ]; then
        NUM_WORKERS=$CUSTOM_WORKERS
        echo "🎯 Using CUSTOM worker count: $NUM_WORKERS workers."
    elif [[ "$TEST_NAME" == *"extreme"* ]] || [[ "$TEST_NAME" == *"stress"* ]]; then
        NUM_WORKERS=6
...
if [ -z "$INPUT_ARG" ]; then
    echo "Usage: $0 [TEST_NAME | SUITE_NAME] [WORKER_COUNT]"
    echo ""
    echo "Arguments:"
    echo "  TEST_NAME     - Name of a specific test to run"
    echo "  SUITE_NAME    - Name of a test suite (load, performance, stress, all)"
    echo "  WORKER_COUNT  - Optional: Number of k6 workers (default: auto-detect)"
    echo ""
    echo "Examples:"
    echo "  $0 trading-scenario           # Run with default 2 workers"
    echo "  $0 trading-scenario 10        # Run with 10 workers"
    echo "  $0 load                       # Run all load tests with default workers"
    echo ""
...
if [ ! -z "$WORKER_COUNT" ]; then
    echo "Worker count: $WORKER_COUNT (custom)"
else
    echo "Worker count: auto-detect"
fi
echo ""

# Loop through tests
for TEST in $TESTS_TO_RUN; do
    run_test $TEST $WORKER_COUNT
